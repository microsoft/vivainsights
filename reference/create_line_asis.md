...
This function creates a line chart directly from the aggregated /
...
`create_line_asis()` and the values are rendered as they are passed into
the function. The only requirement is that a `date_var` is provided for
the x-axis.
...
- line_colour:
  ...
  You can also supply RGB values via
