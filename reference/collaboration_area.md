...
Provides an overview analysis of Weekly Digital Collaboration. Returns
an stacked area plot of Email and Meeting Hours by default. Additional
...
- hrvar:
  ...
  HR Variable by which to split metrics, defaults to `NULL`, but accepts
  any character vector, e.g. "LevelDesignation". If `NULL` is passed,
...
Uses the metrics `Meeting_hours`, `Email_hours`,
`Unscheduled_Call_hours`, and `Instant_Message_hours`.
