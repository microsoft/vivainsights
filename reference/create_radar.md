...
- If `hrvar` is not supplied, the function will call
  ...
  and infer the resulting segment column.
...
- hrvar:
  ...
  Character string specifying the grouping column. Defaults to
  `"Organization"`. If `NULL`, usage segments will be derived via
  [`identify_usage_segments()`](https://microsoft.github.io/vivainsights/reference/identify_usage_segments.md).
...
- agg:
  ...
  String specifying aggregation method. Either `"mean"` (default) or
  `"median"`.
...
- index_mode:
  ...
  - `"minmax"`: scale to \[0, 100\] within observed group ranges per
    metric

- index_ref_group:

  Character string specifying reference group name when
  `index_mode = "ref_group"`.

- na.rm:

  Logical value indicating whether NA rows in required columns are
  removed prior to aggregation. Defaults to `FALSE`.
...
A different output is returned depending on the value passed to
`return`:
...
#> # A tibble: 4 × 5
#>   LevelDesignation Collaboration_hours Email_hours Meeting_hours     n
#>   <chr>                          <dbl>       <dbl>         <dbl> <int>
#> 1 Executive                      101.        101.           98.6    37
#> 2 Junior IC                       99.7        99.5          97.5   136
#> 3 Senior IC                      100.        101.          103.     87
#> 4 Senior Manager                  99.1        99.4         102.     40
```
